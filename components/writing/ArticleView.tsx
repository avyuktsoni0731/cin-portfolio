...
} from '@/components/writing/PostEngagement'
import { ShareBar } from '@/components/writing/ShareBar'

export function ArticleView({ post, url }: { post: Post; url: string }) {
  return (
    <article className="relative min-h-screen bg-background/80 backdrop-blur-[2px]">
      <div
      ...
      <PostEngagementProvider postSlug={post.slug}>
        <div className="relative mx-auto max-w-3xl px-6 pb-28 pt-28 md:pt-32">
          ...
          <header className="mb-10 space-y-6">
            ...
            <h1 className="font-serif text-4xl font-semibold leading-[1.08] tracking-tight text-foreground md:text-5xl md:leading-[1.06]">
              {post.title}
            </h1>
          ...
          {post.coverImage ? (
            <figure className="mb-12 -mx-2 overflow-hidden rounded-sm border border-border/35 sm:-mx-0">
              <Image
